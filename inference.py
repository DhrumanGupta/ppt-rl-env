...
try:
    ...
    try:
        ...
        AgentToolInvocation = Any
        build_openai_tools = None
        parse_tool_invocation = None
        tool_invocation_to_action = None
        PptAgentAction = None
        PptAgentEnvironment = None
    ...
    def _safe_build_openai_tools() -> list[dict[str, Any]]:
        try:
            if callable(build_openai_tools):
    ...
    OPENAI_TOOLS = _safe_build_openai_tools()
    ...
    SYSTEM_PROMPT = textwrap.dedent(
        ...
        - For accent bars use type: accent_bar with a root-level color field: color_hex.
    ...
    def _default_save_path(observation: Any) -> str:
        ...
        return _FALLBACK_SAVE_PATH

    def _make_action(action_type: str, payload: dict[str, Any]) -> Any:
        try:
            if PptAgentAction is not None:
                return PptAgentAction(action_type=action_type, payload=payload)
        except Exception:
            pass
        return SimpleNamespace(
            action_type=action_type, slide_index=None, payload=payload
        )

    def _fallback_save_action(observation: Any) -> tuple[Any, str, Any]:
        payload = {"path": _default_save_path(observation)}
        action = _make_action("save_presentation", payload)
        ...
        invocation = SimpleNamespace(tool_name="save_presentation", arguments=payload)
        return action, action_str, invocation
    ...
    def choose_action(
        client: Any, observation: Any, history: list[dict[str, Any]], max_steps: int
    ) -> tuple[Any, str, Any]:
        metadata = _observation_metadata(observation)
        messages = [
        ...
        try:
            if client is None or not OPENAI_TOOLS:
                return _fallback_save_action(observation)
            ...
            invocation, _ = _extract_tool_invocation(message)
            _validate_tool_choice(invocation, observation)
            action = tool_invocation_to_action(
                invocation,
                default_save_path=_default_save_path(observation),
            )
            return (
                action,
                json.dumps(
    ...
    def run_task(env: Any, client: Any, difficulty: str, max_steps: int) -> None:
        rewards: list[float] = []
        ...
        try:
            ...
            for step in range(1, max_steps + 1):
                ...
                try:
                    ...
                    action, action_str, invocation = _fallback_save_action(observation)

                try:
                    observation = env.step(action)
                    reward = float(getattr(observation, "reward", 0.0) or 0.0)
                    done = bool(getattr(observation, "done", False))
                    error = getattr(observation, "last_action_error", None)
                    rewards.append(reward)
                    history.append(
                        _history_entry_from_tool_call(invocation, observation)
                    )
                    steps_taken = step
                    log_step(
                        step=step,
            ...
            success = bool(
                ...
                and not getattr(observation, "last_action_error", None)
                and float(getattr(observation, "score", 0.0) or 0.0) > 0.0
            )
            score = float(getattr(observation, "score", 0.0) or 0.0)
    ...
    def main() -> None:
        try:
            ...
            try:
                ...
                client = None
            try:
                if PptAgentEnvironment is not None:
